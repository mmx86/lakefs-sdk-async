...
async with lakefs_sdk_async.ApiClient(configuration) as api_client:
    ...
    api_instance = lakefs_sdk_async.StagingApi(api_client)
    ...
    try:
        # generate an address to which the client can upload an object
        api_response = await api_instance.get_physical_address(repository, branch, path, presign=presign)
