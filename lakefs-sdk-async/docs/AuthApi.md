...
async with lakefs_sdk_async.ApiClient(configuration) as api_client:
    ...
    external_principal_creation = lakefs_sdk_async.ExternalPrincipalCreation() # ExternalPrincipalCreation | 

    try:
        # attach external principal to user
        await api_instance.create_user_external_principal(user_id, principal_id, external_principal_creation)
    except Exception as e:
...
async with lakefs_sdk_async.ApiClient(configuration) as api_client:
    ...
    try:
        # get external principal of a user
        api_response = await api_instance.get_user_external_principal(user_id, principal_id)
