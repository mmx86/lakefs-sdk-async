...
async with lakefs_sdk_async.ApiClient(configuration) as api_client:
    ...
    try:
        # get run hook output
        api_response = await api_instance.get_run_hook_output(repository, run_id, hook_run_id)
...
async with lakefs_sdk_async.ApiClient(configuration) as api_client:
    ...
    try:
        # list runs
        api_response = await api_instance.list_repository_runs(repository, after=after, amount=amount, branch=branch, commit=commit)
...
async with lakefs_sdk_async.ApiClient(configuration) as api_client:
    ...
    try:
        # list run hooks
        api_response = await api_instance.list_run_hooks(repository, run_id, after=after, amount=amount)
